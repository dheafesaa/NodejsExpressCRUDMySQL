  ...
  if (!req.file) {
    ...
    koneksi.query(
      ...
      (err, rows, field) => {
        ...
        res
          .status(201)
          .json({ success: true, message: "Berhasil insert data!" });
      }
    );
  } else {
    ...
    const data = { ...req.body };
    const querySql =
      "INSERT INTO mahasiswa (nim,nama,tanggal_lahir,alamat,foto) values (?,?,?,?,?);";

    // jalankan query
    koneksi.query(
      querySql,
      [nim, nama, tanggal_lahir, alamat, foto],
      (err, rows, field) => {
        // error handling
        if (err) {
          return res
...
app.put("/api/mahasiswa/:nim", (req, res) => {
  ...
  const queryUpdate =
    "UPDATE mahasiswa SET nama=?,tanggal_lahir=?,alamat=? WHERE nim = ?";
  ...
  koneksi.query(querySearch, req.params.nim, (err, rows, field) => {
    ...
    if (rows.length) {
      ...
      koneksi.query(
        queryUpdate,
        [nama, tanggal_lahir, alamat, req.params.nim],
        (err, rows, field) => {
          // error handling
          if (err) {
            return res
